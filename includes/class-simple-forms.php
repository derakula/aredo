...
if( !class_exists('SimpleForms') ):
	class SimpleForms{
		...
		function SimpleForms($settings=array()){			
			...
			$this->exclude_fast_atts = array('textarea', 'select', 'option', 'optgroup', 'name_', 'get_option', 'wrap_label', 'wrap_field', 'childs', 'child_template');
...
/* 		$atts['value'] = $this->get_value($atts); */		
		$atts['get_option'] = $this->get_value($atts['name'], $atts['get_option']);
		...
		if (is_array($childs)){
			...
			$wrap_label = isset($atts['wrap_label']) ? $atts['wrap_label'] : $this->wrap_label;
			$wrap_field = isset($atts['wrap_field']) ? $atts['wrap_field'] : $this->wrap_field;
			
			return $this->fields[] = array( 'data' => $atts, 'label' => sprintf($wrap_label, $output_label), 'html' => sprintf($wrap_field, $output_field) );
		}
		...
		$get_option = $atts['get_option'];
			...
			return apply_filters('simple_form_field', $output, $atts, $this);
		}
